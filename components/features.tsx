...
import { FeatureProps, DelegateProps } from './types';
...
export const Product = ({ title, text, href, src }: FeatureProps) => {
  return (
    <Link href={href} target="_blank" _hover={{ textDecoration: 'none' }}>
      ...
      >
        <Avatar name={title} src={src} size="sm" showBorder />
        <Heading fontFamily={'Work Sans'} fontSize="xl">{title}&ensp;&rarr;</Heading>
        <Text>{text}</Text>
      </Stack>
    </Link>
  );
